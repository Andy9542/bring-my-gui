...
Follow-up (same day): `watch` now unlinks `watch.seen` before rewriting it (`mark_seen`) — a watermark another uid left was silently not updated under `protected_regular` and URLs came out twice; the harness asserts it (non-root `watch`, then root's). `tests/ci-local.sh` mirrors the CI job locally on the same images, in parallel, flipping `fs.protected_regular` to 2 for the run (sudo on Linux, nsenter into the Docker Desktop VM) and restoring it; READMEs point to it.
